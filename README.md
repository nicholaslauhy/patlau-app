...
- Parent-support chats mirror the website's escalation and close/reopen rules, show quoted-message context and compact Telegram delivery/activity ticks, securely load parent images only on request with a full-screen zoomable viewer, and support confirmed permanent conversation deletion while retaining the Telegram contact for future chats. Telegram's Bot API does not expose a parent's passive read, online/offline, or typing state, so the app does not claim those states.
